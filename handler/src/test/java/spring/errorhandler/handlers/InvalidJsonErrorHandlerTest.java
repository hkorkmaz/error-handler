...
public class InvalidJsonErrorHandlerTest {
    ...
    void it_should_handle_invalid_format_exception() {
        ...
        var messageKey1 = String.format(MessageHelper.INVALID_FIELD_MESSAGE_KEY, "field1");

        when(messageHelper.getMessage(messageKey1)).thenReturn("field1 message");
        when(messageHelper.getMessage(MessageHelper.INVALID_REQUEST_MESSAGE_KEY)).thenReturn("Actual message");
